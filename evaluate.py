...
def parse_arguments():
    ...
    arg_parser.add_argument(
        ...
        nargs="?",
        required=True,
        help="Path to the predicted labels file",
    )
    arg_parser.add_argument(
        "--out_path",
        type=str,
        nargs="?",
        required=True,
        help="Path where to save scores",
    )
    ...
    arg_parser.add_argument(
        ...
        nargs="?",
        required=True,
        help="Path to the summary of the overall experiments",
    )
...
def get_metrics(gold_path, predicted_path):
    ...
    assert len(gold_relations) == len(
        predicted
    ), "Length of gold and predicted labels should be equal."
    ...
    macro = sum(
        [
            elem[1]["f1-score"]
            if elem[0] in label_types.keys() and elem[1]["support"] > 0
            else 0
            for elem in report.items()
        ]
    ) / sum(
        [
            1 if elem[0] in label_types.keys() and elem[1]["support"] > 0 else 0
            for elem in report.items()
        ]
    )
